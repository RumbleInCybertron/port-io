...
export function DoubleLineChart(/*data: DataProps*/) {
  const dates: Date[] = [];
  ...
  // logo.src = '../public/images/Logo_Name.png';
    ...
    afterDraw(chart: any, args: any, pluginOptions: any) {
      ...
      function tooltipPosition(mousemove: any) {
        ...
        if (mousemove.offsetX >= xLeft && mousemove.offsetX <= xRight && mousemove.offsetY >= top && mousemove.offsetY <= bottom) {
          ...
          const dateIndex = dates.indexOf(dateCursor);
...
function crosshairLine(chart: any, mousemove: any, dates: Date[]) {
  ...
  if (coorX >= left && coorX <= right && coorY >= top && coorY <= bottom) {
    ...
    crosshairPoint(chart, mousemove, dates);
  }
  ctx.setLineDash([]);
};
...
function crosshairPoint(chart: any, mousemove: any, dates: Date[]) {
  const { ctx, data, chartArea: { top, bottom, left, right, width, height }, scales: { x, y } } = chart;
...
function zoom(chart: any, mousewheel: any, dates: Date[]) {
  ...
  //   chart.config.options.scales.x.max = dates[dates.length - 1];
  // }
  // zoomedArea(min, max, dates);
  chart.update('none');
}

// function zoomedArea(min: any, max: any, dates: Date[]) {
//   // area
...
//   function zoomedAreaItem(min: any, max: any) {
...
//     mouseCursor('mousemove', e)
//   });

//   function mouseCursor(mousemove: any, myChart: any) {
...
//   function dragStart(drag: any, myChart: any) {
//     let minChart1 = myChart.config.options.scales.x.min;
...
//     function dragMove(myChart: any, dragDelta: any) {
...
//     function dragMoveCenter(myChart: any, dragDelta: any, staticScaleMin: any, staticScaleMax: any) {
//       // starting point
//       const dragStartingPoint = x.getValueForPixel(drag.offsetX);
//       const dayDragStartingPoint = new Date(dragStartingPoint);
//       dayDragStartingPoint.setHours(0, 0, 0, 0);
